...
class StartUI:
    ...
    def __init__(self):
        ...
        self.simLength = tk.Scale(sTray, from_=1000, to=100000, orient='horizontal', length=winH * 3 / 5,
                                  resolution=1000)
        ...
        self.cbNorm = tk.Checkbutton(fbTray, bg='light grey', text='Enable Normal Cars', variable=self.boolNormCar,
                                     command=self.NormalCar)
        self.cbBBCar = tk.Checkbutton(fbTray, bg='light grey', text='Enable Buffer Builders', variable=self.boolBBCar,
                                      command=self.BBCar)
        self.cbAuton = tk.Checkbutton(fbTray, bg='light grey', text='Enable Autonomus Cars', variable=self.boolAutoCar,
                                      command=self.AutoCar)
        ...
        cbDebug = tk.Checkbutton(bTray, bg="light grey", text='Enable Debugging', variable=self.boolDebug,
                                 command=self.debugWarning)
        cbGraphics = tk.Checkbutton(bTray, bg="light grey", text='Enable Graphics', variable=self.boolGraphics,
                                    command=self.graphicsCheck)
    ...
    def debugWarning(self):
        if (not self.boolDebug.get()):
            messagebox.showwarning('Debugging Enabled',
                                   'Debugging will slow down the simulation\n speed and may cause the system to crash.')
